...
static constexpr char RIGHT_FRAME[] = "right_fr3_hand_tcp";
static constexpr char LEFT_FRAME[]  = "left_fr3_hand_tcp";
...
void MTCTaskNode::setupPlanningScene()
{
  ...
  {  // Target box — right arm side (positive y)
    ...
    shape_msgs::msg::SolidPrimitive prim;
    prim.type = prim.BOX; prim.dimensions = {0.06, 0.06, 0.15};
    geometry_msgs::msg::Pose p;
    p.position.x = 0.5; p.position.y = 0.2; p.position.z = 0.075;
    p.orientation.w = 1.0;
    obj.primitives.push_back(prim); obj.primitive_poses.push_back(p);
    obj.operation = obj.ADD;
    psi.applyCollisionObject(obj);
...
mtc::Task MTCTaskNode::createTask()
{
  ...
  cartesian->setStepSize(0.001);   // smaller step → more steps → jump detection works
  cartesian->setMinFraction(0.5);  // accept 50% of requested distance

  // Grasp frame: rotation to align hand z-axis with approach direction.
  ...
  mtc::Stage * lift_stage_ptr = nullptr;
  {
    ...
    // Generate grasp pose + IK
    {
      auto make_grasp_frame = []() -> Eigen::Isometry3d {
          ...
          Eigen::Quaterniond q =
            ...
            Eigen::AngleAxisd(M_PI / 2, Eigen::Vector3d::UnitZ());
          f.linear() = q.matrix();
          f.translation().x() = -0.05;  // shift grasp point 4 cm up from object center
          return f;
      };
    ...
    // Move right arm EEF to Cartesian handover pre-position.
    // Grasp frame orientation:
    //   EEF x-axis → world +Y
    //   EEF y-axis → world +Z
    //   EEF z-axis → world +X
    // Rotation matrix columns = EEF axes expressed in world frame.
    // Resulting quaternion: (w=0.5, x=0.5, y=0.5, z=0.5)
    {
      ...
      pre_pos.pose.orientation.y    = q_orient.y();
      pre_pos.pose.orientation.z    = q_orient.z();


      auto s = std::make_unique<mtc::stages::MoveTo>("right arm -> handover pre-position", sampling);
      s->setGroup(RIGHT_ARM);
      s->setIKFrame(RIGHT_FRAME);
      s->setGoal(pre_pos);
      lift_stage_ptr = s.get();  // handover IK monitors box at this final position
      pick->insert(std::move(s));
    }
  ...
  // ── Open left hand before any left arm motion ────────────────────────────
  {
  ...
  // Allow left hand <-> box collision before Connect so OMPL can plan the left
  // arm to the pre-approach position without the box blocking the path.
  // The handover container re-enables this internally (redundant but harmless).
  {
  ...
  // ── Handover container ───────────────────────────────────────────────────
  // Pattern mirrors the pick container: approach (backward) → IK → grasp.
  // The approach MoveRelative propagates backward from the IK grasp pose,
  // giving Connect "move to handover" a pre-approach state that is clear of
  // the box and reachable from "ready" without collision.
  mtc::Stage * handover_attach_ptr = nullptr;
  {
    ...
    // Approach: left arm moves toward box along hand z-axis (solved backward by MTC)
    {
      auto s = std::make_unique<mtc::stages::MoveRelative>("approach for handover", cartesian);
      s->properties().set("marker_ns", "handover_approach");
      s->properties().set("link", std::string(LEFT_FRAME));
      s->properties().configureInitFrom(mtc::Stage::PARENT, {"group"});
      s->setMinMaxDistance(0.05, 0.15);
      geometry_msgs::msg::Vector3Stamped vec;
      vec.header.frame_id = LEFT_FRAME;
      vec.vector.z = 1.0;  // approach along left hand z-axis
      s->setDirection(vec);
      handover->insert(std::move(s));
    }

    // Generate left-arm grasp pose on the box + IK
    // Monitored stage is lift_stage_ptr (box is at the right arm's pre-handover position)
    {
      auto make_grasp_frame = []() -> Eigen::Isometry3d {
          Eigen::Isometry3d f = Eigen::Isometry3d::Identity();
          Eigen::Quaterniond q =
      ...
      gen->setMonitoredStage(lift_stage_ptr);  // box is at lifted position here
    ...
    // Transfer: right arm releases first, then left arm takes ownership.
    // detach must come before attach — an attached object is not in the collision world,
    // so attachObject on a second link fails unless the first detach restores it there.
    {
    ...
    // Right arm retreats ~+0.2 m in world +y (collision-aware OMPL).
    // Position: approximate (handover pre-pos y=0.1 + 0.2 → y=0.3); exact final pose is acceptable.
    // Orientation: same as the right arm handover pre-position (programmed in the pick container).
    {
      Eigen::Matrix3d R;
      R.col(0) << 0.0,  0.0, 1.0;
      R.col(1) << -1.0, 0.0, 0.0;
      ...
      retreat_pose.pose.position.x    = 0.3;
      retreat_pose.pose.position.y    = 0.3;  // handover y(0.1) + 0.2
      retreat_pose.pose.position.z    = 0.30;
      retreat_pose.pose.orientation.w = q_orient.w();
      retreat_pose.pose.orientation.x = q_orient.x();
      retreat_pose.pose.orientation.y = q_orient.y();
      retreat_pose.pose.orientation.z = q_orient.z();

      auto s = std::make_unique<mtc::stages::MoveTo>("right arm retreat", sampling);
      s->properties().set("group", std::string(RIGHT_ARM));
      s->setIKFrame(RIGHT_FRAME);
      s->properties().set("marker_ns", "right_retreat");
      s->setGoal(retreat_pose);
      handover->insert(std::move(s));
    }

    {
      ...
      // Update handover_attach_ptr to the LAST stage of the handover container.
      // GeneratePlacePose monitors this stage for the robot base state.
      // RIGHT_ARM and RIGHT_HAND here match the handover container's output state
      // (retreat done, right hand open), making Connect "left arm -> place" compatible.
      // The box world position is unchanged (LEFT_ARM didn't move after "attach box -> left").
      handover_attach_ptr = s.get();
      handover->insert(std::move(s));
    }
